...
window.addEventListener('load', (event) => {
    ...
    let shaders = {'mandlebrot': frag_mandelbrot, 'opt1': frag_opt1, 'opt2': frag_opt2};
    let canvas = new webGLCanvas();
    canvas = setShader(canvas, frag_mandelbrot);

    // enable and disable WebGL rendering
    ...
    document.querySelectorAll(".webgl_button").forEach(element => {
        element.addEventListener('click', (event) => {
            event.preventDefault();
            canvas = setShader(canvas, shaders[event.target.id]);
        });
    });
